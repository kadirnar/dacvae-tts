...
def main():
    ...
    parser.add_argument("--utmos", choices=["utmos22", "utmosv2"], help="Also score every output with this UTMOS")
    ...
    if args.candidates > 1 and sentences:
        ...
        selection_bias = warn_judge_overlap(
            ...
            {"asr": judge_asr, "dnsmos": args.dnsmos, "utmos": args.utmos, "speaker": args.speaker_model},
        )
    prompt_audio = {}
    for index, sentence in enumerate(sentences):
    ...
    utmos = load_utmos(args.utmos, args.device) if args.utmos else None
    for row in scored:
        if "error" not in row:
            ...
            if utmos is not None:
                ...
                row["utmos"] = utmos(read_audio(row["audio"], 16000).numpy())
    ...
    summary.update(
        ...
        utmos_model=args.utmos, utmos=float(np.mean([r["utmos"] for r in good])) if utmos is not None and good else None,
