...
| tie | `base-eager`, `x-no-negatives`, `x-pairs-tail`, `x-pairs-char-ctc`, `x-swiglu`, `x-attn-gate`, `x-long-skip`, `ft-w0`, `grpo`, `y-long-skip`, `y-final-adaln`, `y-cond-text-pool`, `y-swiglu`, `y-decay-matrices` |
...
Final evaluation, sampling seeds 42 + 1000 pooled (y-long-skip/step-0020000, y-long-skip/step-0020000-s1000):

| metric | `y-base` | `y-long-skip` | difference [95 % CI] | verdict |
|---|---:|---:|---|---|
| WER % | 4.32 | 3.99 | -0.33 [-1.02, 0.36] | tie |
| CER % | 2.98 | 2.76 | -0.22 [-0.74, 0.30] | tie |
| SIM-o | 0.587 | 0.589 | +0.002 [-0.004, 0.009] | tie |
| DNSMOS | 3.111 | 3.110 | -0.001 [-0.016, 0.014] | tie |
| UTMOS | 2.486 | 2.512 | +0.026 [0.002, 0.050] | win |


**Trajectory** (WER / CER %; * = quick check, first 96 sentences): 5k: 12.3 / 8.9*, 10k: 7.3 / 5.2*, 15k: 5.5 / 3.4*, 20k: 3.9 / 2.9

**Training.** final validation flow 0.6813 (step 20000); 0.155 s/update; 13,518 target frames/update

**Verdict: tie.** No gain beyond the seed spread. WER 3.99 lies between the two base seeds (4.32 / 3.64); CER, SIM-o (0.589 vs 0.587 / 0.571), DNSMOS and UTMOS within them. +0.53M parameters; not adopted.
...
Final evaluation, sampling seeds 42 + 1000 pooled (y-ffn-conv/step-0020000, y-ffn-conv/step-0020000-s1000):

| metric | `y-base` | `y-ffn-conv` | difference [95 % CI] | verdict |
|---|---:|---:|---|---|
| WER % | 4.32 | 4.04 | -0.28 [-0.87, 0.30] | tie |
| CER % | 2.98 | 2.74 | -0.25 [-0.77, 0.28] | tie |
| SIM-o | 0.587 | 0.598 | +0.011 [0.006, 0.016] | win |
| DNSMOS | 3.111 | 3.090 | -0.021 [-0.037, -0.004] | loss |
| UTMOS | 2.486 | 2.415 | -0.071 [-0.093, -0.050] | loss |


**Trajectory** (WER / CER %; * = quick check, first 96 sentences): 5k: 10.8 / 6.5*, 10k: 6.0 / 4.4*, 15k: 4.9 / 3.4*, 20k: 4.2 / 2.8

**Training.** final validation flow 0.6803 (step 20000); 0.188 s/update; 13,518 target frames/update

**Verdict: rejected.** A trade-off, not a gain. SIM-o 0.598 is above both base seeds (0.587 / 0.571) but DNSMOS (3.090 vs 3.111 / 3.112) and UTMOS (2.415 vs 2.486 / 2.506) fall below both, beyond the seed spread; WER 4.04 and CER 2.74 lie within it. Costs +22 % step time (0.188 vs 0.154 s/update). Not adopted.
...
Final evaluation, sampling seeds 42 + 1000 pooled (y-final-adaln/step-0020000, y-final-adaln/step-0020000-s1000):

| metric | `y-base` | `y-final-adaln` | difference [95 % CI] | verdict |
|---|---:|---:|---|---|
| WER % | 4.32 | 4.04 | -0.28 [-0.93, 0.37] | tie |
| CER % | 2.98 | 2.27 | -0.71 [-1.28, -0.15] | win |
| SIM-o | 0.587 | 0.588 | +0.001 [-0.006, 0.007] | tie |
| DNSMOS | 3.111 | 3.128 | +0.017 [0.000, 0.034] | win |
| UTMOS | 2.486 | 2.495 | +0.009 [-0.016, 0.033] | tie |


**Trajectory** (WER / CER %; * = quick check, first 96 sentences): 5k: 12.4 / 9.2*, 10k: 7.5 / 5.2*, 15k: 5.5 / 3.4*, 20k: 4.2 / 2.3

**Training.** final validation flow 0.6810 (step 20000); 0.153 s/update; 13,518 target frames/update

**Verdict: tie.** No clear gain. WER 4.04, SIM-o 0.588 and UTMOS 2.495 lie within the base seeds; CER 2.27 matches the better seed (2.30); DNSMOS 3.128 is +0.016 above both, but both intervals touch 0. +0.10M parameters, no time cost. Not adopted on this evidence.
...
Final evaluation, sampling seeds 42 + 1000 pooled (y-cond-text-pool/step-0020000, y-cond-text-pool/step-0020000-s1000):

| metric | `y-base` | `y-cond-text-pool` | difference [95 % CI] | verdict |
|---|---:|---:|---|---|
| WER % | 4.32 | 4.53 | +0.20 [-0.53, 0.94] | tie |
| CER % | 2.98 | 3.07 | +0.09 [-0.51, 0.69] | tie |
| SIM-o | 0.587 | 0.590 | +0.003 [-0.001, 0.008] | tie |
| DNSMOS | 3.111 | 3.112 | +0.001 [-0.012, 0.015] | tie |
| UTMOS | 2.486 | 2.499 | +0.013 [-0.004, 0.029] | tie |


**Trajectory** (WER / CER %; * = quick check, first 96 sentences): 5k: 12.2 / 8.5*, 10k: 5.5 / 3.3*, 15k: 5.5 / 3.6*, 20k: 4.4 / 2.9

**Training.** final validation flow 0.6809 (step 20000); 0.153 s/update; 13,518 target frames/update

**Verdict: tie.** No gain. WER 4.53 and CER 3.07 sit slightly above both base seeds (4.32 / 3.64; within the spread of y-base); SIM-o 0.590, DNSMOS 3.112 and UTMOS 2.499 match y-base. +0.26M parameters; not adopted.
...
Final evaluation, sampling seeds 42 + 1000 pooled (y-swiglu/step-0020000, y-swiglu/step-0020000-s1000):

| metric | `y-base` | `y-swiglu` | difference [95 % CI] | verdict |
|---|---:|---:|---|---|
| WER % | 4.32 | 4.13 | -0.19 [-0.77, 0.39] | tie |
| CER % | 2.98 | 2.78 | -0.20 [-0.72, 0.32] | tie |
| SIM-o | 0.587 | 0.569 | -0.017 [-0.022, -0.013] | loss |
| DNSMOS | 3.111 | 3.092 | -0.019 [-0.035, -0.003] | loss |
| UTMOS | 2.486 | 2.509 | +0.022 [-0.001, 0.046] | tie |
...
**Verdict: tie.** No gain on the v2 recipe either. WER 4.13, CER 2.78 and UTMOS 2.509 lie within the base seeds; SIM-o 0.569 matches the lower seed; DNSMOS 3.092 is 0.02 below both. Not adopted.
...
Final evaluation, sampling seeds 42 + 1000 pooled (y-decay-matrices/step-0020000, y-decay-matrices/step-0020000-s1000):

| metric | `y-base` | `y-decay-matrices` | difference [95 % CI] | verdict |
|---|---:|---:|---|---|
| WER % | 4.32 | 3.77 | -0.55 [-1.25, 0.15] | tie |
| CER % | 2.98 | 2.11 | -0.87 [-1.47, -0.27] | win |
| SIM-o | 0.587 | 0.581 | -0.006 [-0.010, -0.002] | loss |
| DNSMOS | 3.111 | 3.122 | +0.011 [-0.003, 0.026] | tie |
| UTMOS | 2.486 | 2.480 | -0.006 [-0.024, 0.011] | tie |


**Trajectory** (WER / CER %; * = quick check, first 96 sentences): 5k: 12.3 / 7.7*, 10k: 7.4 / 5.1*, 15k: 5.0 / 3.0*, 20k: 3.7 / 2.0

**Training.** final validation flow 0.6809 (step 20000); 0.153 s/update; 13,518 target frames/update

**Verdict: tie.** Neutral at weight decay 0.01. CER 2.11 is below both base seeds (2.98 / 2.30) by less than their spread; WER 3.77, SIM-o 0.581 and DNSMOS 3.122 lie within the seeds; UTMOS 2.480 slightly below both. The unusual decay of norm gains and biases was not costing quality at this strength; the option matters for stronger decay (y-regularized).
